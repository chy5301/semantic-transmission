...
def batch_sender(
    ...
    try:
        for idx, image_path in enumerate(discovery.images, 1):
            ...
            try:
                ...
                _print(f"  ✓ 已发送到接收端 耗时 {relay_elapsed:.1f}s")
                ...
                metadata["packet_bytes"] = len(edge_bytes) + len(prompt_text.encode("utf-8"))
    ...
    _print(f"  发送成功: {batch_result.success:>10d} 张")
    _print(f"  发送失败: {batch_result.failed:>10d} 张")
