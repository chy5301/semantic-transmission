...
**Files:**
- Modify: `src/semantic_transmission/pipeline/video_relay.py`（`VideoSendStats`、`VideoRelaySender.run`、新增 `_encode_rgb_png` 复用 `_encode_edge_png`）
...
在文件顶部 import 补 `is_keyframe` 与 `TemporalPolicyConfig`：
...
（`build_reference_images` 供 Task 2 用，一并导入。）
...
class _RefRecordingReceiver(BaseReceiver):
    ...
    def process(self, edge_image, prompt_text, seed=None, reference_images=None):
        ...
        return Image.new("RGB", (512, 384), color=(0, 0, 255))
...
在 `src/semantic_transmission/pipeline/video_relay.py`，`VideoRelayReceiver.run` 加 `reference_mode` 参数并前置分派；新增 `_run_temporal`。先改 `run` 签名与开头：
...
- Test: `tests/test_cli_video_relay.py`（追加）
...
- [ ] **Step 1: 写 CLI 参数校验失败测试**

在 `tests/test_cli_video_relay.py` 追加：
...
Expected: 三个新测试 FAIL（选项未定义 / 校验缺失）。
...
uv run semantic-tx video --input resources/<行车视频>.mp4 \
  ...
  --auto-prompt --output output/klein_single/out.mp4
```

记下 `output/klein_single/prompts.json` 与 `output/klein_single/out.mp4`（作 parity 基线）。
...
Expected: 关键帧（透传）MAE≈0；生成帧因同 seed/prompt/policy 应近乎一致（受编码往返轻微影响，平均 MAE 应很小）。记录数值。
