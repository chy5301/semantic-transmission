...
def test_warp_error_identical_is_zero():
    f = _textured()
...
- Produces: `output/poc/klein-phase2/<label>/` 下 `frames/`、`out.mp4`、`results.json`（含 policy / 质量两栏 / 时序两读 / baseline 对照 / 显存 / 耗时）、`DONE`(或 `DONE.partial`) sentinel、`compare/grid_*.png`。
...
def make_grid(orig, canny, restored, out_dir, n=5):
    ...
    for idx in idxs:
        ...
        r = restored[idx] if restored[idx] is not None else Image.new("RGB", o.size, (40, 40, 40))
        tiles = [o, edge, r]
        h = o.size[1]
...
def main(argv=None) -> int:
    ...
    try:
        ...
        # baseline（阶段 1 drop-in）在相同下标上对照
        base_frames = _load_pngs(args.phase1_dir / "klein" / "frames", args.limit)
        if len(base_frames) == len(fixture):
            ...
            log(f"baseline 帧数 {len(base_frames)} 与 fixture 不符，跳过对照")

        make_grid(fixture, canny, outputs, out_dir / "compare", n=args.grid)
    except Exception as e:  # noqa: BLE001
        ok = False
...
Expected: `DONE`（非 partial）；`results.json` 含 `smoke.locked_R`、`run.keyframe_indices` 含 `0`、`quality.delivered` / `quality.generated_only` 均有 ssim，`temporal.delivered` / `temporal.generated_only` / `temporal.static_window_mae`、`baseline` 段齐全；`frames/` 有 6 张、`out.mp4` 存在、`compare/grid_*.png` 存在。若为 `DONE.partial`，读 `fatal_error`/`traceback` 定位并修复后重跑本步。
...
打开 `output/poc/klein-phase2/x_n12/out.mp4` 与 `compare/grid_*.png`，对比阶段 1 `output/poc/klein-ab-phase1/klein/out.mp4`：重点看近静止 120-127 段闪烁是否缓解、是否出现关键帧周期 pop、是否过度冻结（画面不跟真实运动）。
...
- 设计 §5.3 目视网格 → Task 4 `make_grid` ✅
- 设计 §5.4 复用资产 + 多参考 smoke 探显存有界回退 → Task 4 `smoke_probe` + `_load_pngs`/`_load_prompts` ✅
