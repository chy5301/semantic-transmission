...
**Goal:** 给 klein 接收端加"额外参考帧"能力，并在编排层用可配置时序策略（透传关键帧 + prev 链）压制 drop-in 的剧烈帧间漂移，跑 prev-only@N12/N25 实验并量化时间一致性。
...
def build_reference_images(mode: str, prev_output, last_keyframe) -> list:
    ...
    顺序：prev 在前、keyframe 在后（对齐设计 prev+key = [canny, prev, keyframe]）。
...
TemporalPolicyConfig / is_keyframe / build_reference_images（prev-only/prev+key/消融一个旋钮）+
...
- Produces: `output/poc/klein-phase2/<label>/` 下 `frames/`、`out.mp4`、`results.json`（含 policy / work_size / 质量两栏 / 时序两读 / baseline 对照 / 显存 / 耗时）、`DONE`(或 `DONE.partial`) sentinel、`compare/grid_*.png`（orig｜canny｜drop-in｜输出）+ `compare/strip_static_120-127.png`。
...
Start-Process -NoNewWindow -FilePath "uv" -ArgumentList @(
  ...
  "--reference-mode","prev","--keyframe-interval","12","--limit","6","--label","smoke_prev_only_n12"
) -RedirectStandardOutput "output/poc/klein-phase2/smoke.log" -RedirectStandardError "output/poc/klein-phase2/smoke.err"
```

用 Monitor 守候 sentinel 出现（`output/poc/klein-phase2/smoke_prev_only_n12/DONE` 或 `DONE.partial`）。

- [ ] **Step 5: 核对 smoke 产物**

Run（sentinel 出现后）: `cat output/poc/klein-phase2/smoke_prev_only_n12/results.json`
Expected: `DONE`（非 partial）；`results.json` 含 `smoke.locked_R`、`work_size`、`run.keyframe_indices` 含 `0`、`quality.delivered` / `quality.generated_only` 均有 ssim，`temporal.delivered` / `temporal.generated_only` / `temporal.static_window_mae`、`baseline` 段齐全；`frames/` 有 6 张、`out.mp4` 存在、`compare/grid_*.png`（4 列 orig｜canny｜drop-in｜输出）存在。注：`--limit 6` 时近静止窗口 [120,128) 超出范围，`strip_static` 会跳过属正常。若为 `DONE.partial`，读 `fatal_error`/`traceback` 定位并修复后重跑本步。
...
### Task 5: 全量运行 prev-only@N12 / prev-only@N25 + 阶段 2 报告
...
- Produces: `output/poc/klein-phase2/prev_only_n12/` 与 `prev_only_n25/`（各 250 帧结果 + results.json）；阶段 2 报告。

- [ ] **Step 1: 全量跑 prev-only@N12**（≈2 关键帧/秒，长 GPU 任务，脱离跑 + Monitor 守候）
...
Start-Process -NoNewWindow -FilePath "uv" -ArgumentList @(
  "run","python","-m","scripts.poc.klein_ab.run_phase2",
  "--reference-mode","prev","--keyframe-interval","12","--label","prev_only_n12"
) -RedirectStandardOutput "output/poc/klein-phase2/prev_only_n12.log" -RedirectStandardError "output/poc/klein-phase2/prev_only_n12.err"
```

Monitor 守候 `output/poc/klein-phase2/prev_only_n12/DONE`。预计 ~230 生成帧 × ~15-17s ≈ 60-70min。

- [ ] **Step 2: 全量跑 prev-only@N25**（≈1 关键帧/秒，同上脱离跑）

```powershell
$env:PYTHONIOENCODING="utf-8"
Start-Process -NoNewWindow -FilePath "uv" -ArgumentList @(
  "run","python","-m","scripts.poc.klein_ab.run_phase2",
  "--reference-mode","prev","--keyframe-interval","25","--label","prev_only_n25"
) -RedirectStandardOutput "output/poc/klein-phase2/prev_only_n25.log" -RedirectStandardError "output/poc/klein-phase2/prev_only_n25.err"
```

Monitor 守候 `output/poc/klein-phase2/prev_only_n25/DONE`。

- [ ] **Step 3: 核对两路结果均为 DONE**

Run: `ls output/poc/klein-phase2/prev_only_n12/DONE output/poc/klein-phase2/prev_only_n25/DONE && cat output/poc/klein-phase2/prev_only_n12/results.json output/poc/klein-phase2/prev_only_n25/results.json`
Expected: 两个 DONE 均在；results.json 无 `fatal_error`，`failed` 为 0（或极少）。若 partial，据 traceback 修复重跑。

- [ ] **Step 4: 目视核对闪烁改善**

打开 `output/poc/klein-phase2/prev_only_n12/out.mp4`、`compare/strip_static_120-127.png`（orig / drop-in / 输出 三行并排近静止段）与 `compare/grid_*.png`（orig｜canny｜drop-in｜输出）：重点看近静止 120-127 段 输出 相对 drop-in 闪烁是否缓解、是否出现关键帧周期 pop、是否过度冻结（画面不跟真实运动）。
...
```markdown
# 阶段 2 结论报告：klein 参考帧时间一致性补偿 prev-only 方案（2026-07-01）
...
## 0. 结论先行
（prev-only 是否把 drop-in 的剧烈漂移压到可用；N12 vs N25 哪个好；是否需进 prev+key）

## 1. 实验设置
（复用阶段 1 fixture 250 帧 896×496 / 冻结 prompt / seed=0；prev-only@N12、prev-only@N25；baseline=阶段 1 drop-in）

## 2. 时间一致性（决定性指标）
| 指标 | baseline(drop-in) | prev-only@N12 | prev-only@N25 | 解读 |
|---|---|---|---|---|
| 相邻帧 MAE（交付）| | | | |
| 相邻帧 MAE（生成帧间）| | | | |
| warp-error（交付）| | | | |
...
## 3. 质量指标（两栏）
| 指标 | 栏 | baseline | prev-only@N12 | prev-only@N25 |
|---|---|---|---|---|
...
（据设计 §8 口径：压住→进阶段 3 productionize；压不住→是否上 prev+key 消融，再不行回退 Z-Image）

## 7. 验收
（时序 MAE/warp-error 下降？目视闪烁缓解？未过度冻结？）
```

- [ ] **Step 6: Commit 报告 + 暂停待定 prev+key**

```bash
git add docs/test-reports/2026-07-01-klein-video-phase2.md
git commit -m "docs: klein 阶段 2 prev-only 方案（参考帧补偿）结论报告

prev-only@N12 / prev-only@N25 相对阶段 1 drop-in 的时间一致性 + 质量对比，含近静止段闪烁读数。
据设计 §4/§8 口径：跑完暂停，与负责人据本报告定是否上 prev+key（prev_keyframe）消融。"
```

> **暂停点**（设计 §4/§8）：prev-only 结果复盘后再决定是否跑 prev+key（`--reference-mode prev_keyframe`，harness 已支持，无需改码）。prev+key 不在本计划自动执行范围。
...
- 设计 §4 实验矩阵（先 prev-only@N12/N25 后 prev+key）→ Task 4 harness + Task 5 运行；prev+key 留暂停点 ✅
...
- 设计 §5.3 目视网格（orig｜canny｜drop-in｜输出）+ 近静止条带 → Task 4 `make_grid`（4 列）+ `_static_strip` ✅
