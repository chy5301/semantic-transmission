...
**状态**：进行中。**保底版 M1 单机闭环已达成（2026-06-28）**；目标版 klein 关键帧主线 7 月持续推进——H1 结构遵循度 PoC、阶段 2 参考帧时间一致性补偿、阶段 3（一）时序策略毕业到生产管道均已合并 main（见下方「klein 目标版主线进展」）；RIFE 插帧/超分/流式 I/O 尚未启动；**GUI 仍停留在阶段 2 图像级 6 面板，阶段 3 视频流功能（`video` / `video-sender` / `video-receiver`）未同步到 Web GUI**（已列为收口优先项，见下方收官 checklist C）。详细编排、风险兜底、验收口径见 [6 天冲刺规划设计](superpowers/specs/2026-06-21-video-stream-6day-plan-design.md)；技术选型依据见 [视频流技术方案](research/2026-06-21-video-stream-tech-scout.md) 与 [2026-06 综合评估](research/2026-06-reevaluation.md)；测试方案与实测基线见 [测试方案 spec](superpowers/specs/2026-06-28-video-stream-test-plan-design.md)。
...
按交付物倒推，阶段 3 收口还差以下几层（A 保底补票 / B 目标版主链 / C GUI 同步；D 为明确排除项）。速度优化与多场景终裁明确划归阶段 4，避免实时 KPI 拖住收口。

**A. 保底版补票（M1 完整验收遗留）**

- [ ] 真机双机 relay 演示：两台机器、全流程 `--auto-prompt`、VLM 与 klein 分处两机（当前仅单机 loopback，见 PR #67）

**B. 目标版 PoC 主链（核心收尾，存在数据流依赖）**

- [ ] RIFE 插帧接入：关键帧 + 插帧平滑流（前置技术债 #48 frame_postprocess 改帧数需重算 BatchResult 统计）
- [ ] 超分接入：DLSS 式分层最后一层
- [ ] 端到端延迟 / 帧率 / 一致性三指标测量：目标版 PoC 验收口径（测量指标扩展见下节中期：帧间 LPIPS、光流 warping error、漂移斜率）
- [ ] 流式 I/O 雏形：相机/RTSP 输入、编码回流、背压（前置技术债 #49 read_frames 全量解码需流式化）

**C. GUI 同步阶段 3 功能（优先，与阶段 2 的 6 面板对齐）**

阶段 3 新功能目前仅 CLI 可用（`video` / `video-sender` / `video-receiver`），Gradio GUI 仍停留在阶段 2 的图像级 6 面板，视频流能力无可视化入口——目标版 PoC 现场演示与对外展示会因 CLI-only 而打折。按已落地 CLI 命令补齐可视化入口：

- 第一批（CLI 已稳定合并 main，不依赖 B 链，即刻可做——**优先**）：
  - [ ] video→video 单机面板：backend 选择（klein / z-image）、`--auto-prompt`、时序策略 / 关键帧周期（N）参数、逐帧进度与结果预览
  - [ ] 双机视频 relay 演示面板：`video-sender` / `video-receiver` 可视化入口，恢复阶段 2 的双机演示能力（关联 issue #29 接收端监听 Tab、#32 目录选择器）
- 第二批（跟随 B 链插帧/超分/测量落地后补）：
  - [ ] 插帧 / 超分 / 三指标测量结果的可视化展示

**D. 明确划归阶段 4（不作为阶段 3 收口门槛）**

- 速度优化：klein 稳定态 7–9s/帧 → 目标 ~1–1.5s（fp8 / 降步数 / TensorRT，独立轨，阶段 4 有 KPI 承接）
- 帧间一致性主线多场景终裁
- 中期打磨项：Video-Depth-Anything 深度升级、Qwen3-VL-4B 升级、N 运动自适应、质量评估扩展

**推进顺序（按依赖关系）**

1. **GUI 同步第一批**（C）——封装已稳定合并 main 的 `video` / `video-sender` / `video-receiver`，不依赖 B 链，**优先落地**以恢复演示可视化能力；GUI 中的关键帧周期/backend 选择也便于后续 B 链参数调试
2. **RIFE 插帧**（B）——分层流第一个乘数，先接；先清 #48 帧数-统计一致性前置债
3. **超分**（B）——紧接插帧，DLSS 分层第二层，独立性强
4. **端到端三指标测量**（B）——插帧 + 超分接齐后，对完整分层链做延迟/帧率/一致性测量，先定测量口径
5. **流式 I/O 雏形**（B）——把已验证的离线分层管道改造为流式（相机/RTSP + 编码回流 + 背压），先清 #49 流式化前置债
6. **真机双机 relay 演示**（A）——独立补票项，受「两台机器就绪」外部条件约束，不阻塞 B 链；建议在 B 链主体完成后并入收官端到端演示统一做；演示前先修 #61 auto-prompt 速度 bug（priority-high，影响 `--auto-prompt` 全流程体验）
7. **GUI 同步第二批**（C）——跟随 B 链插帧/超分/测量落地后补，把分层链结果纳入 GUI 可视化

**收口判定**：A + B + C 全部勾选，产出目标版 PoC 演示（关键帧 + 插帧 + 超分平滑流，**含 GUI 可视化入口**）+ 端到端延迟/帧率/一致性测量报告，阶段 3 交付物达成，进入阶段 4。
...
- **GUI 阶段 3 同步**：视频流面板（backend 选择 / 时序策略 / 关键帧周期）+ 双机视频 relay 演示入口，与阶段 2 的 6 面板对齐
