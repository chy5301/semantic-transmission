...
按交付物倒推，阶段 3 收口还差以下三层。速度优化与多场景终裁明确划归阶段 4，避免实时 KPI 拖住收口。
...
1. **RIFE 插帧**（B）——分层流第一个乘数，先接；先清 #48 帧数-统计一致性前置债
2. **超分**（B）——紧接插帧，DLSS 分层第二层，独立性强
3. **端到端三指标测量**（B）——插帧 + 超分接齐后，对完整分层链做延迟/帧率/一致性测量，先定测量口径
4. **流式 I/O 雏形**（B）——把已验证的离线分层管道改造为流式（相机/RTSP + 编码回流 + 背压），先清 #49 流式化前置债
5. **真机双机 relay 演示**（A）——独立补票项，受「两台机器就绪」外部条件约束，不阻塞 B 链；建议在 B 链主体完成后并入收官端到端演示统一做；演示前先修 #61 auto-prompt 速度 bug（priority-high，影响 `--auto-prompt` 全流程体验）

**收口判定**：A + B 全部勾选，产出目标版 PoC 演示（关键帧 + 插帧 + 超分平滑流）+ 端到端延迟/帧率/一致性测量报告，阶段 3 交付物达成，进入阶段 4。
