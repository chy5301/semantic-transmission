# 阶段 2 结论报告：klein 参考帧时间一致性补偿（X / C × N12 / N25，2026-07-01，完成于 2026-07-02）
...
1. **参考帧补偿决定性有效**：4 种配置（X/C × N12/N25）的生成帧间相邻帧 MAE 全部落在 **5.1–5.6**，
...
2. **平滑的核心成分是 prev 链**：4 配置生成帧间平滑度几乎一致（prev 上一帧输出锚定相邻帧），说明治闪烁靠
   的是「上一帧输出当参考」，而非关键帧本身。
3. **恒挂关键帧（C）> 仅 prev（X）**：C 在保真（SSIM/LPIPS/PSNR）与近静止段稳定性上一致更优——真实关键帧
   把生成帧持续拉回现实内容。**C@N12 全指标最优**。
4. **关键帧越密（N12）> 越疏（N25）**：N12 保真更好但透传 pop 更多（delivered 抖动略高、码率略高）；N25
   码率低但生成帧离真值更远。
5. **显存/速度**：4 路均在 896×496、峰值 ≤19.78GB / 24GB、失败 0；参考帧越多越慢（X 14–16s、C 17s/生成帧）。
   速度优化是 7 月独立工作，与本质量/一致性验证正交。

**推荐**：klein 主线进阶段 3 productionize，默认配置 **C@N12**（质量最优）或 **X@N12**（时序几乎打平、
快 ~24%、少 0.6GB，速度敏感时选它）；N 是「保真↔码率」旋钮、C/X 是「保真↔算力」旋钮，均可用。
...
| 变量 | reference_mode ∈ {X=`prev`, C=`prev_keyframe`} × keyframe_interval ∈ {12, 25} |
...
| X@N12 | 5.25 | 6.24 | **−76.3%** | 12.90 |
| X@N25 | 5.19 | 6.24 | **−76.6%** | 13.27 |
| **C@N12** | **5.13** | **6.02** | **−76.8%** | **11.13** |
| C@N25 | 5.56 | 6.42 | −74.9% | 13.16 |
| *原始视频对照* | — | — | — | *1.11* |

- **生成帧间平滑度 4 配置基本打平**（5.13–5.56）：prev 链是治闪烁的主力，与 C/X、N 关系不大。
- **近静止段 C@N12 最优（11.13）**：恒挂真实关键帧把该段拉得最稳；仍高于原始 1.11（残余闪烁存在，但较
  baseline 23.2 砍掉 **52%**）。
...
|---|---|---|---|
| X@N12 | 10.40 | 5.25 | 5.15 |
| C@N12 | 9.77 | 5.13 | 4.64 |
| X@N25 | 7.60 | 5.19 | 2.41 |
| C@N25 | 7.66 | 5.56 | 2.10 |

- **透传关键帧带来周期性 pop**（真图↔生成帧外观跳变）：N12（21 帧）比 N25（10 帧）pop 更频、交付抖动更高。
- **C 的恒定锚点略压 pop**（C@N12 9.77 < X@N12 10.40）：每帧都朝真关键帧靠，过渡更缓。
...
| X@N12 | 32.70 | 0.605 | 0.440 | 15.65 |
| X@N25 | 31.29 | 0.551 | 0.507 | 15.48 |
| **C@N12** | **32.80** | **0.638** | **0.402** | **16.43** |
| C@N25 | 31.11 | 0.601 | 0.462 | 16.11 |

- **C@N12 全质量指标最优**：恒挂真实关键帧持续把生成帧拉向真实内容 → 保真最高。
...
|---|---|---|---|---|---|---|
| X@N12 | 229 | 21 | 0 | 14.03 | 19.15GB | 53.5min |
| X@N25 | 240 | 10 | 0 | 15.99 | 19.15GB | 64.0min |
| C@N12 | 229 | 21 | 0 | 17.41 | 19.78GB | 66.5min |
| C@N25 | 240 | 10 | 0 | 17.27 | 19.78GB | 69.1min |

- 参考帧越多越慢：X（2 参考）14–16s、C（3 参考）17s/生成帧；显存 X 19.15GB、C 19.78GB，**均远低于 24GB**，
...
- `strip_static_120-127.png`：近静止段 orig / drop-in / X（或 C）三行并排连续帧条带——最直观看闪烁缓解与 pop。
- `grid_0000/0062/0124/0187/0249.png`：逐帧 `orig｜canny｜drop-in｜X(或C)` 4 列并排。
- `out.mp4` + `frames/`：整段还原视频与逐帧 PNG。
- 四个 label：`x_n12` / `x_n25` / `c_n12` / `c_n25`。
...
- **配置建议**：
  - **质量优先 → C@N12**（`--reference-mode prev_keyframe --keyframe-interval 12`）：全指标最优。
  - **速度/算力优先 → X@N12**：时序几乎打平 C@N12，质量略低，快 ~24%、省 0.6GB。
...
- ✅ 回答裁决点：**参考帧补偿有效、klein 主线成立**；给出 X/C × N 的完整权衡与配置建议。
- ✅ 显存实测：多参考（含 3 参考 C）峰值 ≤19.78GB / 24GB，无 OOM、无分辨率回退。
