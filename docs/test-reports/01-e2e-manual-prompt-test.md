...
> 评估指标：PSNR（像素保真度）、SSIM（结构相似度）、LPIPS（感知距离，越低越好）
...
|---|------|-------------|--------|---------|
| 1 | 峡谷越野 | 12.71 | 0.3134 | 0.4766 |
| 2 | 攀岩越野 | 12.81 | 0.2652 | 0.4993 |
| 3 | 丛林小路 | 15.98 | 0.3606 | 0.4246 |
| 4 | 雪地松林 | 10.79 | 0.1192 | 0.6155 |
| 5 | 山间公路 | 17.44 | 0.7072 | 0.3907 |
| 6 | 草原雪山 | 14.22 | 0.5424 | 0.5819 |
| — | **均值** | **13.99** | **0.3847** | **0.4981** |
| — | **标准差** | **2.21** | **0.1912** | **0.0798** |

### 指标解读

- **PSNR 13.99 dB**：像素级偏差较大，符合语义传输预期——不追求逐像素还原
- **SSIM 0.38**：结构相似度中低，还原图保持了大致构图但细节不同
- **LPIPS 0.50**：感知距离中等，视觉效果与主观评价"良好"基本吻合
- **最佳样本**：#5 山间公路（PSNR=17.44, SSIM=0.71, LPIPS=0.39）——边缘简洁、构图规整
- **最差样本**：#4 雪地松林（PSNR=10.79, SSIM=0.12, LPIPS=0.62）——prompt 场景误判导致全局色偏

### 量化与主观评价的一致性

量化指标与逐张主观评价高度一致：#5（良好）和 #6（优秀）的指标最好，#4（一般）指标最差。LPIPS 作为感知指标比 PSNR/SSIM 更贴合人类感知——#6 草原雪山主观评为"优秀"但 LPIPS=0.58，原因是色调整体偏冷导致感知距离偏大，而人类更关注语义一致性。
